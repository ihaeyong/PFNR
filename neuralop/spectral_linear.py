...
class GetSubnetFaster(torch.autograd.Function):
    @staticmethod
    def forward(ctx, scores, zeros, ones, sparsity, idx):
        ...
        if idx < 2:
            if False:
                ...
                eps = torch.randn_like(scores) * 1e-1 # 1e-1
    ...
    def backward(ctx, g):
        ...
        #g_ones = g * (scores + zeros * scores)
        g_zeros = g * (scores + ones * scores)

        return g, None, None, None, None

def _contract_dense(x, weight_real, weight_imag, separable=False, 
                    weight_mask_real=None, weight_mask_imag=None, device=None):
    order = tl.ndim(x)
    ...
    # abcd, abcd -> abcd : hadamard
    #eq_hadamard= ''.join(x_syms) + ',' + ''.join(x_syms) + '->' + ''.join(x_syms)

    #if not torch.is_tensor(weight):
    #    weight = weight.to_tensor()

    if weight_mask_real is not None and weight_mask_imag is not None:
        ...
        #return torch.mm(x[:,:,0,0], weight[:,:,0,0])[:,:,None,None]
    else:
        return tl.einsum(eq, x, weight)
...
class FactorizedSpectralLinear(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n_modes, incremental_n_modes=None, bias=False,
        ...
        if joint_factorization:
            ...
            if self.subnet:
                ...
                self.scale_list = [self.scale if i < 2 else 1.0 for i in range(self.n_weights_per_layer*n_layers)]
                self.sparsity_list = [sparsity if i < 2 else sparsity for i in range(self.n_weights_per_layer*n_layers)]
        ...
        self._contract = get_contract_fun(self.weight_real[0], implementation=implementation, separable=separable)
    ...
    def init_mask_parameters(self, real=True, imag=True):
        ...
        if real:
            for w_m in self.w_m_real:
                ...
                if self.bias is not None:
                    fan_in, _ = nn.init._calculate_fan_in_and_fan_out(self.w_m_real)
        ...
        if imag:
            for w_m in self.w_m_imag:
                nn.init.kaiming_uniform_(w_m, a=math.sqrt(5))
                if self.bias is not None:
                    fan_in, _ = nn.init._calculate_fan_in_and_fan_out(self.w_m_imag)
                    bound = 1 / math.sqrt(fan_in)
                    nn.init.uniform_(self.b_m, -bound, bound)
    ...
    def forward(self, x, indices=0, output_shape=None, weight_mask_real=None, weight_mask_imag=None, mode='train'):
        ...
        for i, boundaries in enumerate(itertools.product(*mode_indexing)):
            ...
            if mode == 'train':
                ...
                weight_mask_real = self.weight_mask_real[idx]
                weight_mask_imag = self.weight_mask_imag[idx]
...
class FactorizedSpectralConv1d(FactorizedSpectralLinear):
